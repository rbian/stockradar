...
> AI驱动的A股智能选股系统 — 300只沪深300 · 年化29.1% · 多Agent协作 · 自进化
...
> **2025年更稳**：年化提升57%，Sharpe提升65%，回撤收窄13%。策略持续有效。
...
```
/top       → 36因子评分Top10 (含持仓标记📦)
/nav       → 净值+收益+回撤
/report    → 今日日报 (行情+新闻+诊断)
分析茅台    → 个股深度分析 (9层:行情→技术→基本面→行业→新闻→LLM估值)
持仓建议    → 自动评分→建仓Top10
诊断       → 策略医生: 持仓5日涨跌+异常预警
因子       → IC追踪: 因子有效性Top/Bottom5
假设       → LLM因子假设生成+自动验证
周报/月报   → 完整周期报告
净值图     → 净值曲线图
回测       → 历史回测结果
...
src/factors/    ← 36因子引擎
src/data/       ← QVeris + BaoStock 适配器 + 行业分类 + 新闻情绪
src/simulator/  ← 净值追踪 (建仓/调仓/止损)
src/evolution/  ← 因子IC追踪 + 策略医生 + 假设生成
scripts/        ← Bot入口 + 回测 + 数据初始化 + BaoStock每日更新
