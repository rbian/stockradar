...
### 2026-06-19 (周五) 策略迭代+代码审查日

**代码审查发现:**
- 🔴🔴🔴 **CRITICAL: 市场广度过滤从未生效** — 检查`pct_chg`列但数据列为`pctChg`(大写C)，且2026年5月起`change_pct`和`pctChg`全为NaN（新浪适配器未填充）
  - 影响: bot在系统性下跌日(如6/18日75%股票下跌)仍正常买入，市场广度保护完全失效
  - 根因: `update_daily_from_sina`只写OHLCV，不计算涨跌幅
  - 已修复: 1) 运行时自动检测列名+从close计算 2) 数据源修复 3) 历史数据回填
  - commit: 4fd3c7f, 78ae663

- 🔴 **波动率状态检测同样失效** — 同样的`pct_chg`列名问题
  - 影响: 高波动环境从未提高买入门槛
  - 已修复，commit: 1af16ab

- 🟡 **组合回撤Tier3过于宽松** — 回撤-19.7%时仍允许scale=0.3的买入
  - 当前胜率13.3%，持续亏损中
  - 已收紧: Tier3 scale 0.3→0.15 signal 75→80, 新增Tier3.5(-18%): scale=0.05 signal=85
  - commit: 1b80068

**改进实施 (4项):**
1. ✅ **🔴 市场广度过滤修复** — 列名统一检测+自动从close计算pct_change
2. ✅ **🔴 新浪适配器修复** — 数据源现在正确填充pre_close/change_pct/pctChg
3. ✅ **🔴 历史数据回填** — 378706/378709行change_pct现已非空(之前337821)
4. ✅ **🟡 回撤保护收紧** — Tier3 scale 0.3→0.15, 新增Tier3.5深度回撤冻结

**复盘驱动:**
- 主要错误模式: "买入失误"12次(avg后续5日跌6.9%)
- 根因分析: 市场广度过滤失效是最大因素 — 在系统性下跌日仍正常买入
- 本次修复直接解决了这个根本问题

**GitHub学习:**
- **QuantConnect/Lean** (9k+⭐): Maximum Drawdown Recovery概念
  - 学到: 深度回撤时应指数级缩减仓位而非线性
  - 已实现: Tier3.5深度冻结(scale=0.05)
